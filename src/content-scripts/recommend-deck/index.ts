...
export async function refreshDeckPanel() {
  ...
  const existingPanel = document.querySelector('.deck-panel');
  if (existingPanel) {
    existingPanel.remove();
  }

  // Reset the panelAdded flag
  panelAdded = false;

  // Call the checkPanelExists function to add the panel again
  await checkPanelExists();
}
